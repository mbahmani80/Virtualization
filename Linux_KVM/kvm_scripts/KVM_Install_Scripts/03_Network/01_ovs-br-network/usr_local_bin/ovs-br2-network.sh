#!/bin/bash
#===============================================================================
# Script Name: ovs-br2-network.sh
...
# Date: 2025-11-21
# Usage: ./ovs-br2-network.sh {create|delete|recreate|start|stop|status|help}
# Version: 2.2
#-------------------------------------------------------------------------------
#===============================================================================
...
start() {
  echo "[INFO] Starting br2 configuration..."
  ...
  # Set MTU for interfaces and bridge
  ip link set dev $IF1 mtu 9000
  ip link set dev $IF2 mtu 9000
  ip link set dev $BRIDGE mtu 9000

  # Enable promiscuous mode on the interfaces and bridge
  ip link set dev $IF1 promisc on
  ip link set dev $IF2 promisc on
  ip link set dev $BRIDGE promisc on

  echo "[INFO] br2 is configured."
}

stop() {
  echo "[INFO] Stopping br2 configuration..."

  # Bring down the interfaces and bridge
  ip link set $IF1 down
  ip link set $IF2 down
  ip link set $BRIDGE down

  echo "[INFO] br2 is cleaned up."
}
